...
object Lists {
  ...
  assert(
    pack(List(1, 2, 2, 3, 3, 3)) == List(1, 2, 2, 3, 3, 3)
      .groupBy(identity)
      .values
      .toList
  )
  ...
  def modifiedEncode[T](items: List[T]): List[Any] =
    encode(items).map { case (length, elem) =>
      if (length == 1) elem else (length, elem)
    }
  ...
  def drop[T](n: Int, x: List[T], offset: Int = 0): List[T] = x match {
    case Nil => Nil
    case x :: xs =>
      if (offset == n - 1) drop(n, xs, 0) else x :: drop(n, xs, offset + 1)
  }
  assert(
   ...
   */
  def randomSelect[T](n: Int, x: List[T]): List[T] = if (n == 0) Nil
  else {
  ...
  assert(
    "peach".toList.combinations(2).toList == combinations(2, "peach".toList)
  )
  ...
  def lsortFreq[T](x: List[List[T]]) =
    x.groupBy(_.size).toList.sortBy(_._2.size).map(_._2).flatten
}
