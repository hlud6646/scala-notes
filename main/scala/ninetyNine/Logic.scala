...
object Logic {
  ...
  def gray(n: Int): List[List[Boolean]] =
    if (n == 1) List(List(false), List(true))
    else {
      val old = gray(n - 1)
  ...
  def frequencyTable(s: String) =
    s.toList.groupBy(identity).mapValues(_.size).toList.sortBy(_._2)
  def combine(t: List[Tree]) = if (t.size < 2) t
  else {
    val f =
      Fork(t(0), t(1), chars(t(0)) ++ chars(t(1)), weight(t(0)) + weight(t(1)))
  ...
  def encode(s: String) = {
    ...
    def until[T](p: List[T] => Boolean, reduce: List[T] => List[T])(
        ts: List[T]
    ): List[T] = {
