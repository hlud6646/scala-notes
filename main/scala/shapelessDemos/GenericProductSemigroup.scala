...
object GenericProductSemigroup {
  ...
  object Semigroup {
    ...
    def instance[T](f: (T, T) => T) = new Semigroup[T] { def combine(x: T, y: T) = f(x, y) }
    implicit val intSemigroup:    Semigroup[Int]    = instance { (x, y) => x + y }
    implicit val stringSemigroup: Semigroup[String] = instance { (x, y) => x + y }
    implicit val hnilSemigroup:   Semigroup[HNil]   = instance { (_, _) => HNil }
    implicit def hlistSemigroup[H, T <: HList](
      implicit
      hSg: Semigroup[H], 
      tSg: Semigroup[T]): Semigroup[H :: T] = instance {
        case (h1 :: t1, h2 :: t2) =>  hSg.combine(h1, h2) :: tSg.combine(t1, t2)
    }
    implicit def productSemigroup[T, R](
      implicit
      gen: Generic[T] { type Repr = R },
       sg: Semigroup[R]): Semigroup[T] = instance { case (x, y) => { gen.from(sg.combine(gen.to(x), gen.to(y))) }
    }
  }
