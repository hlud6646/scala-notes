...
object GenericProductSimple extends App {
  ...
  object Weighbridge {
    ...
    implicit def pairEncoder[S, T](implicit sWb: Weighbridge[S], tWb: Weighbridge[T]): Weighbridge[(S, T)] = 
  ...
  val catWeighbridge = Weighbridge.instance[Cat] { 
  ...
  implicit val hnilWeighbridge:  Weighbridge[HNil] = Weighbridge.instance {_ => 0}
  implicit def hlistWeighbridge[H, T <: HList](implicit hWb: Weighbridge[H], tWb: Weighbridge[T]): Weighbridge[H :: T] = 
    Weighbridge.instance {
      case h :: t => hWb.weight(h) + tWb.weight(t)
    }
   ...
   */
  implicit def genericProductWeighbridge[A, R](
    implicit 
    gen: Generic[A] { type Repr = R },
    wb:  Weighbridge[R]): Weighbridge[A] = Weighbridge.instance {
      a => wb.weight(gen.to(a))
    }
  ...
  object Length {
    ...
    implicit def hlistLength[H, T <: HList](implicit hLength: Length[H], tLength: Length[T]): Length[H :: T] = 
      instance { case h :: t => hLength.length(h) + tLength.length(t) }
    implicit def productLength[T, R](
      implicit 
      gen: Generic[T] { type Repr = R }, 
      len: Length[R]): Length[T] = instance { x => len.length(gen.to(x)) }
