...
object BinaryTrees extends App {
  ...
  case class Fork[+T](value: T, left: Tree[T], right: Tree[T]) extends Tree[T] {
    ...
    def atLevel(i: Int) = if (i == 1) this :: Nil else left.atLevel(i - 1) ::: right.atLevel(i - 1)
    def isSymmetric = this == this.flip
    def isBalanced = left.size == right.size
    def isHbalanced = List(-1, 0, 1).contains(left.height - right.height) && left.isHbalanced && right.isHbalanced
    def insert[U >: T <% Ordered[U]](x: U) = 
      if (this.value == x) this
  ...
  object Tree {
    ...
    def trees1(n: Int): List[Tree[String]] = if (n == 0) List(Empty) else (for {
      i <- 0 to n - 1
      a <- trees1(i)
      b <- trees1(n - 1 - i)
    ...
    val allTrees: LazyList[List[Tree[String]]] = List(Empty) #:: ints.drop(1).map(n => {
      (for {
        i <- 0 until n
        a <- allTrees(i)
        b <- allTrees(n - 1 - i)
   ...
   */
  def symmetricBalancedTrees(n: Int) = allTrees(n) filter {t => t.isSymmetric && t.isBalanced} 
